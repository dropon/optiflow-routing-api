...
> ReachableAreasResponse GetReachableAreas(ctx, id).MaximumPollingPeriod(maximumPollingPeriod).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ReachableAreasAPI.GetReachableAreas(context.Background(), id).MaximumPollingPeriod(maximumPollingPeriod).Execute()
	if err != nil {
